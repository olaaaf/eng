...
async def get_action(
    model_id, db: DBHandler = Depends(get_db), logger=Depends(get_logger)
):
...
async def submit_score(
    ...
    for step in data.steps:
        state = torch.tensor(
            [step.mario_x, step.mario_y, step.frame], dtype=torch.float32
        )
...
async def log_message(
    data: LogData, db: DBHandler = Depends(get_db), logger=Depends(get_logger)
):
